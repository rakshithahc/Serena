...
@SuppressLint({ "SetJavaScriptEnabled", "WorldWriteableFiles", "WorldReadableFiles" })
public class search_webview extends Activity {
	Calendar cal;
	...
	protected void onResume() {
		super.onResume();
		if (isfirstEntry) {
			// int searchtime= cal.get(Calendar.HOUR) * 60 * 60 + cal.get(Calendar.MINUTE) *  60+ cal.get(Calendar.SECOND) * 1000;
			...
			if (!Network_Available.hasConnection(catWebCtx) && sec > 60) {
				Intent m = new Intent(search_webview.this,
				...
				Intent m = new Intent(search_webview.this, SplashScreen.class);
				m.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK);
				startActivity(m);
				finish();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Log.v("Search url with time in webview", final_req_url);
		...
		wb.setWebViewClient(new WebViewClient() {
			...
			public boolean shouldOverrideUrlLoading(WebView view,
				...
				if (urlNewString
					...
					search_webview.queueItemForAttachment.clear();
					// onBackPressed();
	...
	private void attachmentthings() {
		...
		attachment.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Intent intent = new Intent(search_webview.this,
						SelectOption.class);
